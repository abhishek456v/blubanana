...
for (const b of buttons) {
  ...
  if (visible && (name?.toLowerCase().includes('add') || text?.toLowerCase().includes('add') || name?.toLowerCase().includes('brand') === false)) {
    // print all visible for now
  }
  if (visible) console.log(JSON.stringify({ name, text: text?.slice(0,50) }))
}
