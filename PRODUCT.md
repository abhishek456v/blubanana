...
**9 — Marketing website. Phase A and B done.** `web/`: static HTML generated by
`web/build.mjs`, no framework and nothing to install on the host. The palette,
the radii, the gradients and the typeface come from `constants/design.ts`, so
the site and the app are recognisably one product.
...
one pixel of the app in it. That is what every generated landing page looks
like, and it is why this one was thrown away rather than edited. The rebuild is
structured on a reading of honeybook.com — homepage, feature pages, audience
pages, comparison pages, free tools — with one thing deliberately not copied.

**The social-proof layer is not reproduced.** HoneyBook's trust rests on
customer counts, dollars processed and review scores. CreatorDesk has no
customers yet, and inventing that shape would be the same fabrication §3
refuses in the pricing. What stands in its place is all verifiable: real
screenshots of the product, the actual invoice it generates, the price and the
launch-seat count read live from the database, and the compliance facts.

Screenshots come from a **demo workspace** (`scripts/seed-demo.mjs`) with
invented brands and deterministic figures — never a real one. Publishing a real
workspace would put a brand's commercial terms on the open internet and imply
client relationships that do not exist.

Still to build: the eight product pages, three audience pages, three comparison
pages and five free tools — the last of which compile from `lib/tax.ts`,
`lib/invoiceHtml.ts` and `lib/profileCardHtml.ts` themselves, so the calculator
on the website and the one in the app cannot drift apart.

The contact details are still placeholders, and the build fails on them by
design rather than leaving a note in a README.
