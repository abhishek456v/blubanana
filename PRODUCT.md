...
|---|---|---|
| **Marketing website** | Public. What the product does, pricing, sign-up, the five pages Razorpay requires. Sends people to the platform | **Built** — `web/`, six pages; product and acquisition pages pending |
...
**9 — Marketing website. Built, and rebuilt.** `web/`: thirteen static pages
generated by `web/build.mjs`, no framework and nothing to install on the host.
...
(`web/src/ui.mjs`). The app's UI is not approved, so a screenshot would date the
...
**Five free tools**, compiled from `lib/tax.ts` and `constants/gst.ts` through
