...
**Refunds: 30 days, money back, no questions about usage.** Chosen rather than
defaulted. A creator who subscribes in a quiet month and does not open it until
the next campaign is the honest failure case for a product bought in advance,
and seven days does not cover them. After 30 days a term already provided is not
refunded and is not pro-rated. Errors and prolonged outages sit outside the
window entirely.
...
|---|---|---|
| **Marketing website** | Public. What the product does, pricing, sign-up, the five pages Razorpay requires. Sends people to the platform | **Built** — `website/`, 22 pages |
...
**9 — Marketing website. Built, and rebuilt.** `website/`: thirteen static pages
generated by `website/build.mjs`, no framework and nothing to install on the host.

It is what unblocks payments. Razorpay will not activate a merchant account
until the site carries Contact with a working phone number, Pricing, Terms,
Privacy and Cancellation & Refunds. All five exist.

The first version was a claim grid, six text cards describing the app with not
one pixel of the app in it. The second showed the app and was too dark, too
wordy, and pinned to an interface that is not finished. What stands now:

**Light by default, dark on request.** White ground, one accent, soft shadows,
a lot of space between sections. The app's near-black gradient identity is the
right call for a tool someone sits inside for an hour and the wrong one for a
two minute visit from a stranger.

**No screenshots at all.** Every interface on the site is drawn in HTML and CSS
(`website/src/ui.mjs`). The app's UI is not approved, so a screenshot would date the
site every time a screen moves; and a real workspace carries a creator's brands,
rates, bank details and dates, which do not belong on a public page. The
drawings carry none of that, and where a quantity matters it is a proportion.

**One line per idea.** If a section needs a second sentence to land, the first
sentence is wrong.

**No em or en dashes**, enforced by the build rather than by proofreading.

**The launch counter stays off until it flatters.** The 500 cap is real and
still in the database, and the offer is stated plainly; how many places are
*left* appears only once four fifths have gone. Early on that number reads as
"nobody has signed up", which is true and unhelpful.

**Five free tools**, compiled from `platform/lib/tax.ts` and `platform/constants/gst.ts` through
esbuild, so the calculator on the website and the one in the app are the same
function. Advance tax, TDS, GST, rate and engagement rate.

**The comparison argues the work, not the tax.** Foreign tools are competent
software written for a country where brands pay on time and the year starts in
...
**Twenty two pages now**, including the eight feature pages, an overview, the
five free tools and a tools index. The Features menu builds itself from the
feature pages, so a new one appears in the navigation and the footer by
